...
class SimBox extends SimTransform{
  ...
  public boolean intersectsSphere(SimSphere sphere) {
    ...
    if ( isRotated() == false ) {
      ...
      print(u.dot(C) + "|" + u3.dot(A) + " | Result: ", u.dot(C) <= u1.dot(point) && u2.dot(point) <= u3.dot(A));
      print(v.dot(A) + "|" + v3.dot(B) + " | Result: ", v.dot(A) <= v1.dot(point) && v2.dot(point) <= v3.dot(B));
      print(w.dot(D) + "|" + w3.dot(A) + " | Result: ", w.dot(D) <= w1.dot(point) && w2.dot(point) <= w3.dot(A));
      println();
      
      if ((v.dot(A) <= v1.dot(point) && v2.dot(point) <= v3.dot(B))){
            println("Success!");
        return true;
      }
    }
